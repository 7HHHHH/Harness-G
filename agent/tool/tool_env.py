...
def step_batch(envs: List['ToolEnv'], action_texts: List[str]):
    ...
    for tool_name, args_list in tool_groups.items():
        ...
        # try:
        # Try batch execution
        if hasattr(tool, "batch_execute_with_envs"):
            batch_results = tool.batch_execute_with_envs(args_list, envs_list)
        else:
...
class ToolEnv:
    ...
    @property
    def is_harness_g(self) -> bool:
        return any(getattr(tool, "is_harness_g", False) for tool in self.tools)

    def set_context(self, question: str = "", data_source: str = "", prompt: Any = None, extra_info: Dict = None):
        extra_info = extra_info or {}
        resolved_question = question or extra_info.get("question") or self.question
        if not resolved_question and prompt is not None:
            resolved_question = self._extract_question_from_prompt(prompt)
        self.question = str(resolved_question or "")
        self.data_source = str(data_source or self.data_source or "")
        return self
    ...
    def _extract_question_from_prompt(prompt: Any) -> str:
        if isinstance(prompt, list):
            ...
            text = str(prompt or "")
        matches = re.findall(r"Question:\s*(.*)", text, flags=re.DOTALL)
        if not matches:
            return ""
        question = matches[-1].strip()
        question = re.split(r"<\|im_end\|>|<\|im_start\|>", question)[0].strip()
        return question
...
{"query": <statement-to-search>}
    ...
    def reset_tracking_variables(self):
        ...
        self._snc_steps = []  # Optional SNC side-channel payload per step (aligned 1:1 with _actions)
        self._snc_last_step = None  # Transient: set by the Harness-G tool for the current step
    ...
    def get_tracking_variables(self) -> Dict:
        ...
        return {
            ...
            "snc_steps": getattr(self, "_snc_steps", []),
        }
    ...
    def _update_tracking_variables(
        ...
        # Accumulate the optional SNC side-channel for this step, aligned 1:1 with _actions.
        # None when SNC is disabled or the step did not execute the tool. Clear the transient
        # afterwards so a stale value cannot leak into a later non-executing step.
        try:
            self._snc_steps.append(getattr(self, "_snc_last_step", None))
        except AttributeError:
            self._snc_steps = [getattr(self, "_snc_last_step", None)]
        self._snc_last_step = None
    ...
    def copy(self):
        ...
        env._snc_steps = deepcopy(getattr(self, "_snc_steps", []))
        env._snc_last_step = None
        return env
