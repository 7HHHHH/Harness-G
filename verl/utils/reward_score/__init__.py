...
def _default_compute_score(data_source, solution_str, ground_truth, extra_info=None):
    if os.environ.get("HARNESS_G_REWARD", "").lower() in {"1", "true", "yes"}:
        from . import harness_g_qa
        return float(harness_g_qa.compute_score(solution_str, ground_truth, extra_info=extra_info))
...
def _default_compute_score_format(data_source, solution_str, extra_info=None):
    if os.environ.get("HARNESS_G_REWARD", "").lower() in {"1", "true", "yes"}:
        from . import harness_g_qa
        return float(harness_g_qa.compute_score_format(solution_str, extra_info=extra_info))
    from . import qa_em_and_format
...
def _default_compute_score_answer_em(data_source, solution_str, ground_truth, extra_info=None):
    if os.environ.get("HARNESS_G_REWARD", "").lower() in {"1", "true", "yes"}:
        from . import harness_g_qa
        return float(harness_g_qa.compute_score_em(solution_str, ground_truth, extra_info=extra_info))
    from . import qa_em_and_format
...
def _default_compute_score_answer_f1(data_source, solution_str, ground_truth, extra_info=None):
    if os.environ.get("HARNESS_G_REWARD", "").lower() in {"1", "true", "yes"}:
        from . import harness_g_qa
        return float(harness_g_qa.compute_score_f1(solution_str, ground_truth, extra_info=extra_info))
    from . import qa_em_and_format
...
def _default_compute_score_format_answer(data_source, solution_str, ground_truth, extra_info=None):
    if os.environ.get("HARNESS_G_REWARD", "").lower() in {"1", "true", "yes"}:
        from . import harness_g_qa
        return float(harness_g_qa.compute_score_format_answer(solution_str, ground_truth, extra_info=extra_info))
    from . import qa_em_and_format
    res = qa_em_and_format.compute_score_format_answer(solution_str, ground_truth)
