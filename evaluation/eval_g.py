...
def _make_client():
    ...
    if not api_key:
        raise RuntimeError("missing OPENAI_API_KEY or openai_api_key.txt")
...
"""
    ...
    def score_extraction(metric):
        try:
            ...
            score_match = re.search(r"<score>\s*(\d{1,2})\s*</score>", content, flags=re.IGNORECASE)
            ...
            if not score_match:
                raise ValueError(f"failed to parse score from: {content[:200]}")
